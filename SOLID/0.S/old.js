...
var _musicToggleElement = document.querySelector('#music-toggle');
var _musicSliderElement = document.querySelector('#music-slider');
var _engineToggleElement = document.querySelector('#engine-toggle');
var _addFuelForm = document.querySelector('#add-fuel-form');
var _addFuelInput = document.querySelector('#add-fuel-input');
var _fuelLevelElement = document.querySelector('#fuel-level');
var _milesElement = document.querySelector('#miles-value');
var _audioElement = document.querySelector('#car-music');
var car = new Car(100);
_musicToggleElement.addEventListener('click', function () {
    if (car.musicLevel === 0) {
        car.turnMusicOn();
        _musicSliderElement.value = car.musicLevel.toString();
        _musicToggleElement.innerText = 'Turn music on';
        return;
    }
    _musicToggleElement.innerText = 'Turn music off';
    car.turnMusicOff();
});
//I use input instead of change, because then the value changes when I move the mouse, not only on release
_musicSliderElement.addEventListener('input', function (event) {
    var target = (event.target);
    car.musicLevel = target.value;
    _audioElement.volume = car.musicLevel / 100;
    //@todo when you are repeating the same text over and over again maybe we should have made some constants for it? Can you do improve on this?
    _musicToggleElement.innerText = car.musicLevel ? 'Turn music off' : 'Turn music on';
});
_engineToggleElement.addEventListener('click', function () {
    if (car.engineStatus) {
        car.turnEngineOff();
        _engineToggleElement.innerText = 'Turn engine on';
        return;
    }
    _engineToggleElement.innerText = 'Turn engine off';
    car.turnEngineOn();
});
_addFuelForm.addEventListener('submit', function (event) {
    event.preventDefault();
    car.addFuel(Number(_addFuelInput.value));
    _fuelLevelElement.innerText = car.fuel.toString();
});
setInterval(function () {
    car.drive();
    //while it looks like both lines below are the same there is a subtle difference (you could put breakpoints here to see the difference):
    // this <cast> will only tell TypeScript that the value is a string, but the actual variable in JS is not changed in any way: it is in reality still a number
    _milesElement.innerText = (car.miles);
